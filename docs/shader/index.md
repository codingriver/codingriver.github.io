---
title: "Shader"
date: "2026-03-21"
tags:
  - Shader
  - 图形学
categories:
  - shader
comments: false
---
# Shader

这是 Shader 分类栏目页。

## 迁移说明

- 本栏目承接原 Hugo 站点中 `content/posts/shader/` 下的全部文章
- 文章已整体迁移到当前目录
- 图片统一放到 `image/文章名/` 目录中

## 已迁移文章

- [Bloom实现](Bloom实现.md)
- [Gamma和Linear及sRGB说明](Gamma和Linear及sRGB说明.md)
- [ShaderLearn-(一)漫反射](ShaderLearn-(一)漫反射.md)
- [ShaderLearn-(三)RampTex](ShaderLearn-(三)RampTex.md)
- [ShaderLearn-(二)镜面反射](ShaderLearn-(二)镜面反射.md)
- [Shader光照模型-漫反射](Shader光照模型-漫反射.md)
- [Shader光照模型-高光反射](Shader光照模型-高光反射.md)
- [Shader学习-1.1基础概要](Shader学习-1.1基础概要.md)
- [Shader学习-前言](Shader学习-前言.md)
- [Shader水波纹效果](Shader水波纹效果.md)
- [Shader法线纹理](Shader法线纹理.md)
- [Shader法线贴图](Shader法线贴图.md)
- [Shader法线贴图（世界空间）](Shader法线贴图（世界空间）.md)
- [Shader消融特效](Shader消融特效.md)
- [Shader纹理动画](Shader纹理动画.md)
- [UnityShader常用函数](UnityShader常用函数.md)
- [模糊实现](模糊实现.md)
- [边缘光和流光特效](边缘光和流光特效.md)
- [边缘光特效](边缘光特效.md)
