...
title: "IGG开发笔记"
subtitle: "IGG开发笔记"
date: 2022-03-28T14:59:24+08:00
author: "codingriver"
...
> ![](../images/2023-04-11-16-51-18.png)  
...
1. 网络数据 写入`mReceiveBuffer`中
    > ![](../images/2023-04-23-15-44-13.png)
1. 然后`Copy`到`mReadQueueBuffer`（网络数据拆包）进行传递
    > ![](../images/2023-04-23-15-46-39.png)
1. 然后使用`packetData`进行派发，`packetData`是新的buffer
    > ![](../images/2023-04-23-16-15-48.png)  
    > ![](../images/2023-04-23-15-52-57.png)  
    >
1. 通过`Socket`异步回调线程将`packetData` 存储到`private LockFreeQueue<MemoryStream> mReceivePacketQueue`队列中，然后通过`unity`主线程进行派发,在主线程中进行`Decode`
    > ![](../images/2023-04-23-16-18-00.png)
1. `Decode`时，`MemoryStream.ToArray()`进行拷贝，生成新的buffer
    >![](../images/2023-04-23-15-55-03.png)  
    > ![](../images/2023-04-23-15-56-41.png)  
    > ![](../images/2023-04-13-11-07-09.png)  
    >![](../images/2023-04-13-11-11-06.png)  
    >![](../images/2023-04-13-10-50-05.png)  
1. `Decode`首先进行解压缩，解压缩时先把原始buffer`Copy`到`newBuffer`，`newBuffer`是新的buffer
    > ![](../images/2023-04-23-15-58-56.png)
1. 然后进行解压缩`decomBuffer`，解压缩`decomBuffer`是新的buffer
    > ![](../images/2023-04-23-16-00-10.png)
    > ![](../images/2023-04-23-16-00-42.png)
1. 解压后进行解密`outBuffer`，解密是新的buffer
    > ![](../images/2023-04-23-16-03-28.png)
1. 解密后进行解包`unpack_data`,解包是新的buffer
    > ![](../images/2023-04-23-16-04-44.png)
1. 解包后解析对象`Package`
1. 解析对象后进行解析对象`GateMessage`,`GateMessage`对象有新的`byte[]`字段,该字段有新的buffer
    ...
    > ![](../images/2023-04-23-16-05-56.png)
    > ![](../images/2023-04-23-16-10-48.png)
...
1. 调用频繁的业务协议使用缓存池进行处理
    ...
    > ![](../images/2023-04-23-16-39-24.png)


>优化中期参照：  
> *编号为1的测试数据，每帧解析30001协议50次的开销，测试数据为`Map_ObjectInfo.request`,数据包含有60个`MapObjectInfo`对象*   
> ![](../images/2023-04-12-21-57-40.png)  
> *上面结果是将所有协议都做缓存，包括`MapObjectInfo`内部字段引用的协议*
> ![](../images/2023-04-13-14-55-45.png)  
...
> 优化最终结果:
>*编号为1的测试数据，每帧解析30001协议50次的开销，测试数据为`Map_ObjectInfo.request`,数据包含有60个`MapObjectInfo`对象*   
> ![](../images/2023-04-23-15-37-32.png)   
...
> ![](../images/2023-04-23-17-23-13.png)   
>游戏中优化前后对比:  
> 优化前每一次调用 `GC` 是 `255kb`
...
> PoolMgr的缓存池测试
>
>![](../images/2023-04-25-16-40-28.png)